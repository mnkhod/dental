            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-3 mb-4">
                        <form action="<?php echo e(url('/reception/time/week/doctor')); ?>" method="post">
                            <select class="form-control">
                                <option><?php echo e($role->staff->name); ?></option>
                                <?php $__currentLoopData = $roles->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role_op): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php if($role->id == $role_op->id): ?>
                                        <?php else: ?>
                                        <option><?php echo e($role_op->staff->name); ?></option>
                                    <?php endif; ?>
                ...
                <table class="table table-responsive text-center table-bordered">
                    <tr>
                        ...
                        <?php $s = 0?>
                        <?php for($d = 0; $d < 7; $d++): ?>
                            <th><?php echo e(date('Y-m-d', strtotime("+".$d." Days"))); ?></th>
                        <?php endfor; ?>
                    </tr>
                    <?php for($i = 0; $i<6; $i++): ?>
                        <tr>
                            ...
                            <?php for($d = 0; $d < 7; $d++): ?>
                                <?php if($shift = $shifts->where('date', date('Y-m-d', strtotime("+".$d." Days")))->first()): ?>
                                        ...
                                        <?php if($appointment = $shift->appointments->where('start', 9+$i)->first()): ?>
                                            <td height="90px" rowspan="<?php echo e($appointment->end - $appointment->start); ?>">
                                                <button class="btn btn-primary btn-block text-left"
                                                    ...
                                                    <?php echo e($appointment->name); ?><br><span><?php echo e($appointment->phone); ?></span>
                                                </button>
                                            </td>
                                    ...
                                    <?php else: ?>
                                        <td height="90px" style="background-color: #bcbcbc">
                                            <button class="btn btn-primary btn-block text-left hidden"
                                                    style="border-radius: 20px; height: 100%;">Ээлж байхгүй
                                                <br><span>байна</span></button>
                                        </td>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <td height="90px" style="background-color: #bcbcbc">
